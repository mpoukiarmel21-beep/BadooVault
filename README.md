...
Multi-container isolation tweak for **Badoo** (`com.badoo.Badoo`) on
...
> Personal use only. BadooVault injects into an IPA you already own; it does
> not distribute Badoo. The base IPA is copyrighted — do not redistribute it.

## How it works

BadooVault is a **substrate-free** dylib injected into the Badoo binary and
...
dead ones, ad-hoc `codesign`s, and publishes `BadooVault.ipa` as a release
...
```bash
# 1. Host the decrypted Badoo IPA as a release asset (tag e.g. v1.0-ipa)
# 2. Trigger the build against that tag:
gh workflow run build.yml -f ipa_url=v1.0-ipa
# 3. Download BadooVault.ipa from the build-N release, install via Sideloadly.
```
...
Tweak/Makefile       LIBRARY_NAME = BadooVault
