...
static NSDictionary<NSString *, NSDictionary<NSString *, NSString *> *> *IVL10nTable(void) {
    ...
    dispatch_once(&once, ^{
        ...
        t = @{
            ...
            @"create.footer"    : @{ @"fr" : FR(@"Chaque conteneur est un téléphone isolé et répond ces informations à Badoo."), @"en" : @"Each container is an isolated phone and answers this information to Badoo.", @"es" : @"Cada contenedor es un teléfono aislado y responde esta información a Badoo.", @"de" : @"Jeder Container ist ein isoliertes Telefon und antwortet Badoo mit diesen Informationen.", @"it" : @"Ogni contenitore è un telefono isolato e risponde queste informazioni a Badoo.", @"pt" : @"Cada contêiner é um telefone isolado e responde essas informações ao Badoo." },
            ...
            @"swipe.detectHint" : @{ @"fr" : FR(@"Détection best-effort : le bot agit sur l'UI de Badoo (like/dislike + popup « match »). Selon la version de Badoo, un réglage sur l'appareil peut être nécessaire."), @"en" : @"Best-effort detection: the bot acts on Badoo's UI (like/dislike + match popup). Depending on the Badoo version, a device setting may be required.", @"es" : @"Detección de mejor esfuerzo: el bot actúa sobre la UI de Badoo (like/dislike + popup de match). Según la versión de Badoo, puede ser necesario un ajuste.", @"de" : @"Best-effort-Erkennung: Der Bot agiert auf der Badoo-UI (Like/Dislike + Match-Popup). Je nach Badoo-Version kann eine Geräteeinstellung nötig sein.", @"it" : @"Rilevamento best-effort: il bot agisce sull'UI di Badoo (like/dislike + popup match). A seconda della versione di Badoo potrebbe servire un'impostazione.", @"pt" : @"Detecção best-effort: o bot age na UI do Badoo (like/dislike + popup de match). Conforme a versão do Badoo, pode ser necessária uma configuração." },
